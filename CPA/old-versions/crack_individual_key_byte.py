...
########################################
best_subkey = np.argmax(max_correlation_coefficients)

plt.figure()
plt.plot(range(256), max_correlation_coefficients)
plt.xlabel("Subkey value (0-255)")
plt.ylabel("Maximum Pearson correlation coefficient")
plt.title("DPA Attack on AES: Correlation per Subkey Hypothesis")

# Optional: highlight the best subkey candidate
plt.axvline(best_subkey, linestyle="--")
plt.text(
    best_subkey, max_correlation_coefficients[best_subkey],
    f"  best subkey = {best_subkey}",
    va="bottom"
)
