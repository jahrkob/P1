...
traces = np.load('data/generated-by-chatgpt/2/traces_full.npy')
textins = np.load('data/generated-by-chatgpt/2/textins_full.npy')
...
for subkey_index in trange(16, desc="Subkey Index"):
    ...
    for subkey in range(0x00, 0xff + 1):
        max_correlation_coefficients[subkey] = max(abs(
            optimized_calculate_correlation_coefficients(subkey, subkey_index)
        ))
