#!/usr/bin/env python3
import numpy as np

# =========================================
# CONFIGURATION
# =========================================
# DATASET_TYPE options:
#   "full"        -> all 16 bytes leak at POIs: 10 + 10*byte_idx
#   "single"      -> only SINGLE_BYTE_INDEX leaks at SINGLE_BYTE_POI
#   "single_scan" -> only SINGLE_BYTE_INDEX leaks, but we scan all POIs
DATASET_TYPE = "full"    # <--- set this for your current situation

TRACES_PATH   = "data/generated-through-script/traces_full.npy"
TEXTINS_PATH  = "data/generated-through-script/textins_full.npy"
TRUE_KEY_PATH = "data/generated-through-script/key_full.npy"  # or None if you don't want to load it

# Single-byte leakage settings (used in "single" and "single_scan" modes)
SINGLE_BYTE_INDEX = 0       # which plaintext/key byte leaks
SINGLE_BYTE_POI   = 100     # only used in DATASET_TYPE="single"


# =========================================
# Load data
# =========================================
traces = np.load(TRACES_PATH)
textins = np.load(TEXTINS_PATH)

num_traces, trace_length = traces.shape
print(f"[+] Loaded {num_traces} traces of length {trace_length}")
print(f"[+] Dataset type: {DATASET_TYPE}")

# Try to load true key if available (for verification)
true_key = None
if TRUE_KEY_PATH is not None:
    try:
        true_key = np.load(TRUE_KEY_PATH)
        print(f"[+] Loaded true key from {TRUE_KEY_PATH}")
    except FileNotFoundError:
        print(f"[!] {TRUE_KEY_PATH} not found, will only output recovered key.")
else:
    print("[!] TRUE_KEY_PATH is None, skipping true key loading.")


# =========================================
# AES S-box and Hamming weight
# =========================================
SBOX = np.array([
    
], dtype=np.uint8)

HW = np.array([bin(x).count("1") for x in range(256)], dtype=np.float32)


# =========================================
# Pearson correlation
# =========================================
def pearson_corr(x, y):
    x = x.astype(np.float64)
    y = y.astype(np.float64)
    x = x - x.mean()
    y = y - y.mean()
    num = np.dot(x, y)
    den = np.sqrt(np.dot(x, x) * np.dot(y, y))
    if den == 0:
        return 0.0
    return num / den


# =========================================
# Helpers
# =========================================
def key_to_hex(k):
    return " ".join(f"{b:02x}" for b in k)

def key_to_ascii(k):
    return "".join(chr(b) if 32 <= b <= 126 else "." for b in k)


# =========================================
# CPA attack
# =========================================
if DATASET_TYPE == "full":
    # All 16 bytes leak at known POIs: 10 + 10*byte_idx
    recovered_key = np.zeros(16, dtype=np.uint8)

    for byte_idx in range(16):
        poi = 10 + 10 * byte_idx

        if poi >= trace_length:
            raise ValueError(f"POI {poi} out of range for trace length {trace_length}")

        samples = traces[:, poi]

        best_corr = -1.0
        best_guess = 0

        print(f"\n[+] Attacking subkey byte {byte_idx} at POI {poi}")

        for kguess in range(256):
            pbytes = textins[:, byte_idx]
            inter = SBOX[pbytes ^ kguess]
            hyp = HW[inter]
            corr = abs(pearson_corr(hyp, samples))

            if corr > best_corr:
                best_corr = corr
                best_guess = kguess

        recovered_key[byte_idx] = best_guess
        print(f"    -> best guess: 0x{best_guess:02x}  (|corr| = {best_corr:.5f})")

elif DATASET_TYPE == "single":
    # Only one byte leaks at a known POI
    recovered_key = np.zeros(16, dtype=np.uint8)

    byte_idx = SINGLE_BYTE_INDEX
    poi = SINGLE_BYTE_POI

    if poi >= trace_length:
        raise ValueError(f"POI {poi} out of range for trace length {trace_length}")

    samples = traces[:, poi]

    best_corr = -1.0
    best_guess = 0

    print(f"\n[+] Attacking single leaking byte {byte_idx} at POI {poi}")

    for kguess in range(256):
        pbytes = textins[:, byte_idx]
        inter = SBOX[pbytes ^ kguess]
        hyp = HW[inter]
        corr = abs(pearson_corr(hyp, samples))

        if corr > best_corr:
            best_corr = corr
            best_guess = kguess

    recovered_key[byte_idx] = best_guess
    print(f"    -> best guess for byte {byte_idx}: 0x{best_guess:02x}  (|corr| = {best_corr:.5f})")

elif DATASET_TYPE == "single_scan":
    # Only one byte leaks, but we don't know the POI -> scan all time samples
    recovered_key = np.zeros(16, dtype=np.uint8)

    byte_idx = SINGLE_BYTE_INDEX
    pbytes = textins[:, byte_idx]

    print(f"\n[+] Scanning POIs for leaking byte {byte_idx} over all {trace_length} samples")

    # Precompute hypothetical leakage matrix: shape (num_traces, 256)
    key_guesses = np.arange(256, dtype=np.uint8)
    inter_all = SBOX[pbytes[:, None] ^ key_guesses[None, :]]  # (N, 256)
    hyp_all = HW[inter_all]                                   # (N, 256)

    hyp_all = hyp_all.astype(np.float64)
    hyp_mean = hyp_all.mean(axis=0)           # (256,)
    hyp_centered = hyp_all - hyp_mean         # (N, 256)
    hyp_var = np.sum(hyp_centered ** 2, axis=0)  # (256,)

    best_overall_corr = -1.0
    best_overall_guess = None
    best_overall_poi = None

    best_corr_per_poi = np.zeros(trace_length)
    best_guess_per_poi = np.zeros(trace_length, dtype=np.int32)

    for poi in range(trace_length):
        samples = traces[:, poi].astype(np.float64)
        samples_centered = samples - samples.mean()
        s_var = np.sum(samples_centered ** 2)

        if s_var == 0:
            continue

        # numerator for all 256 key guesses at once: (256,)
        num = np.dot(samples_centered, hyp_centered)  # (256,)
        den = np.sqrt(hyp_var * s_var)               # (256,)
        corr_all = np.abs(num / den)

        best_idx = np.argmax(corr_all)
        best_corr = corr_all[best_idx]

        best_corr_per_poi[poi] = best_corr
        best_guess_per_poi[poi] = best_idx

        if best_corr > best_overall_corr:
            best_overall_corr = best_corr
            best_overall_guess = best_idx
            best_overall_poi = poi

    print(f"\n[+] Best overall POI: {best_overall_poi}")
    print(f"    -> best key guess at that POI: 0x{best_overall_guess:02x} (|corr| = {best_overall_corr:.5f})")

    # Also print top 10 POIs
    top_k = 10
    idx_sorted = np.argsort(-best_corr_per_poi)[:top_k]
    print(f"\n[+] Top {top_k} POIs by max correlation:")
    for rank, poi in enumerate(idx_sorted, start=1):
        print(f"   #{rank}: POI {poi:3d}  best_guess=0x{best_guess_per_poi[poi]:02x}  "
              f"|corr|={best_corr_per_poi[poi]:.5f}")

    # Fill recovered key with best guess at best_overall_poi
    recovered_key[byte_idx] = best_overall_guess

else:
    raise ValueError(f"Unknown DATASET_TYPE: {DATASET_TYPE!r}")


# =========================================
# Print results
# =========================================
print("\n==============================")
print("Recovered key (hex):")
print(key_to_hex(recovered_key))
print("Recovered key (ASCII-ish):")
print(key_to_ascii(recovered_key))

if true_key is not None:
    print("\nTrue key (hex):")
    print(key_to_hex(true_key))
    print("True key (ASCII-ish):")
    print(key_to_ascii(true_key))

    matches = np.sum(recovered_key == true_key)
    print(f"\nByte-wise matches: {matches}/16")
    if matches == 16:
        print("[+] Full key recovered correctly 🎉")
    else:
        print("[!] Not all bytes match (note which bytes are actually supposed to leak).")
